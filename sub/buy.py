...
class buyBySelenium:
        ...
        for product in products:
            ...
            for product in products:
                name_element = product.find_element(
                    By.XPATH, './/div[contains(@class, "name")]')
                link_element = product.find_element(By.XPATH, './/a')
                product_name = name_element.text
                product_link = link_element.get_attribute('href')
                product_list.append(
                    {'name': product_name, 'link': product_link})
    ...
    def cart(cls):
        # 장바구니로 이동
        browse_cart = cls.driver.find_element(
            By.XPATH, '/html/body/div[2]/div[1]/header/section/div[1]/ul/li[2]/a/span[1]/img')
        browse_cart.click()
        time.sleep(10)
